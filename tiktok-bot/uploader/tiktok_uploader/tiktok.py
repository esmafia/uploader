...
def upload_to_tiktok(video_file, session):
        ...
        # Use file size from disk — avoids loading the whole file into memory.
        ...
        # upload chunks — stream from disk one chunk at a time (no full-file buffer)
        ...
        with open(video_path, "rb") as f:
                while True:
                        ...
                        crc = crc32(chunk)
                        crcs.append(crc)
                        url = f"https://{upload_host}/{store_uri}?partNumber={part_number}&uploadID={upload_id}&phase=transfer"
                        headers = {
                                "Authorization": video_auth,
